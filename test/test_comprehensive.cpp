...
bool test_memory_leak_simulation() {
    ...
    std::ofstream test_log("/tmp/test_memory_leak.tmp");
    ...
    if (std::ifstream check_file("/tmp/test_memory_leak.tmp"); check_file) {
        std::string line;
        int line_count = 0;
    ...
    // Clean up
    std::remove("/tmp/test_memory_leak.tmp");
...
bool test_unicode_and_special_chars() {
    ...
    std::ofstream test_log("/tmp/test_unicode.tmp");
    ...
    // Clean up
    std::remove("/tmp/test_unicode.tmp");
...
bool test_very_long_lines() {
    ...
    std::ofstream test_log("/tmp/test_long_lines.tmp");
    ...
    // Test that the file was created
    std::ifstream check_file("/tmp/test_long_lines.tmp");
    TEST_ASSERT(check_file.good(), "Long lines test file should be created");
    
    // Clean up
    std::remove("/tmp/test_long_lines.tmp");
...
bool test_malformed_valgrind_lines() {
    ...
    std::ofstream test_log("/tmp/test_malformed.tmp");
    test_log << "==12345== Invalid read of size 4\n";
    ...
    // Clean up
    std::remove("/tmp/test_malformed.tmp");
...
bool test_nested_templates_and_complex_types() {
    ...
    std::ofstream test_log("/tmp/test_complex_types.tmp");
    test_log << "==12345== Invalid read of size 4\n";
    ...
    // Clean up
    std::remove("/tmp/test_complex_types.tmp");
...
bool test_file_permissions() {
    ...
    std::ofstream test_log("/tmp/test_permissions.tmp");
    test_log << "==12345== Test file with permissions\n";
    test_log.close();
    
    // Change permissions to read-only
    chmod("/tmp/test_permissions.tmp", 0444);
    ...
    chmod("/tmp/test_permissions.tmp", 0644);
    std::remove("/tmp/test_permissions.tmp");
...
bool test_concurrent_access_simulation() {
    ...
    for (int i = 0; i < 10; ++i) {
        std::string filename = "/tmp/test_concurrent_" + std::to_string(i) + ".tmp";
...
bool test_memory_efficiency() {
    ...
    std::ofstream test_log("/tmp/test_memory_efficiency.tmp");
    ...
    // Clean up
    std::remove("/tmp/test_memory_efficiency.tmp");
...
bool test_vglog_filter_integration() {
    ...
    std::ofstream test_log("/tmp/test_vglog_integration.tmp");
    test_log << "==12345== Invalid read of size 4\n";
    test_log << "==12345==    at 0x401234: main (test.cpp:10)\n";
    test_log << "==12345== \n";
    test_log << "==12345== Invalid read of size 4\n";
    ...
    // Test that the file was created
    std::ifstream check_file("/tmp/test_vglog_integration.tmp");
    TEST_ASSERT(check_file.good(), "vglog-filter integration test file should be created");
    
    // Clean up
    std::remove("/tmp/test_vglog_integration.tmp");
...
bool test_stream_processing_edge_cases() {
    ...
    std::ofstream test_log("/tmp/test_stream_edge.tmp");
...
bool test_marker_trimming_edge_cases() {
    ...
    std::ofstream test_log1("/tmp/test_marker_begin.tmp");
    ...
    std::ofstream test_log2("/tmp/test_marker_end.tmp");
    ...
    // Test with no marker
    std::ofstream test_log3("/tmp/test_marker_none.tmp");
    ...
    // Clean up
    std::remove("/tmp/test_marker_begin.tmp");
    std::remove("/tmp/test_marker_end.tmp");
    std::remove("/tmp/test_marker_none.tmp");
